...
class Share:
    ...
    def gen_ris_sta(self):
        ...
        result += '上涨百分比 >= ' + str(self.upStartPct) + '% 的k线共 ' + str(
            len(data[data.pct_chg > self.upStartPct])) + ' 个' + '\n'
    ...
    def gen_ris_trend(self, pct, time):
        ...
        result += str(time) + self.timeUnit + '内上涨幅度 >= ' + str(pct) + '%的概率: ' + str(
            kp2dig(len(trendData[trendData.last_weeks < time][trendData.pct >= pct]) / len(trendData))) + ' %' + '\n'
    ...
    def gen_down_sta(self):
        ...
        result += '下跌百分比 <= ' + str(self.downStartPct) + '% 的k线共 ' + str(
            len(data[data.pct_chg < self.downStartPct])) + ' 个' + '\n'
    ...
    def volatility(self):
        latitude = max(self.basData.pct_chg.max(), abs(self.basData.pct_chg.min()))
        latitude = int(latitude) + 1
        statisticData = {'pct_chg': [], 'pct_chg_str': [], 'num': [], 'percentage':[]}


        for pct in range(-latitude, latitude + 1):
            statisticData['pct_chg'].append(pct)
            statisticData['pct_chg_str'].append(str(pct) + '-' + str(pct + 1))
            statisticData['num'].append(0)

        for pct in self.basData.pct_chg.fillna(0):
            if (pct < 0):
                pct -= 1
